...
export class DAOService {
  ...
  public userData!: any
  constructor(
    public theProgressService: ProgressService
  ) { }
        ...
        (snapshot) => {
          ...
          switch (snapshot.state) {
            ...
            case 'running':
              ...
              this.theProgressService.status = 'in progress'
              this.theProgressService.percentageDone = 'Upload is ' + progress.toFixed(2) + '% done', `Uploaded ${ snapshot.bytesTransferred } from ${ snapshot.totalBytes }`;
              break;
          }
        },
        (error) => {
          // Handle unsuccessful uploads
          this.theProgressService.status = 'error'
        },
        () => {
          this.theProgressService.status = 'completed'
